...
export MONASCA_LOG_API_WAIT_RETRIES=${MONASCA_LOG_API_WAIT_RETRIES:-"24"}
export MONASCA_LOG_API_WAIT_DELAY=${MONASCA_LOG_API_WAIT_DELAY:-"5"}
...
cd /monasca-log-api
export OS_TEST_PATH=./monasca_log_api_tempest/tests/log_api

if [ ! -r .testrepository ]; then
  testr init
fi

OSTESTR_REGEX=${OSTESTR_REGEX:-"monasca_log_api_tempest"}
